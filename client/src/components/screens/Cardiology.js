...
const Cardiology = () => {
    const [cardiologies, setCardiologies] = useState([]);
    // const [error, setError] = useState(null);
    // const [heading, setPageHeading] = useState('');
    const [pageTitle, setPageTitle] = useState('');

    useEffect(() => {
        const fetchCardiologyData = async () => {
            try {
                const response = await axios.get('/api/auth/cardiology');
                const cardiologyData = response.data;

                const cardiologiesWithImages = cardiologyData.map(cardiology => ({
                    ...cardiology,
                    image1: `data:image/png;base64,${cardiology.image1}`,
                    image2: `data:image/png;base64,${cardiology.image2}`
                }));

                setCardiologies(cardiologiesWithImages);

                if (cardiologiesWithImages.length > 0) {
                    setPageTitle(cardiologiesWithImages[0].title);
                    // setPageHeading(cardiologiesWithImages[0].heading); // Set the heading here
                }
            } catch (error) {
                console.error('Error fetching cardiology data:', error);
                // setError('Failed to fetch cardiology data');
            }
        };
                ...
                <section className="service-details-wrap ptb-100">
                    <div className="container">
                        <div className="row gx-5">
                            {cardiologies.map(cardiology => (
                                <div key={cardiology._id} className="col-xl-8">
                                    <article>
                                        <div className="post-img">
                                            <div className="row">
                                            <h2 dangerouslySetInnerHTML={{ __html: pageTitle }}></h2>
                                                <div className="col-md-6">
                                                    <a className="single-service-img" data-fancybox="gallery" href={cardiology.image1}>
                                                        <img src={cardiology.image1} alt={cardiology.title} />
                                                    </a>
                                                </div>
                                                <div className="col-md-6">
                                                    <a className="single-service-img" data-fancybox="gallery" href={cardiology.image2}>
                                                        <img src={cardiology.image2} alt={cardiology.title} />
                                                    </a>
                                                </div>
                                            </div>
                                        </div>
                                        <div className="post-para" dangerouslySetInnerHTML={{ __html: cardiology.content }} />
                                    </article>
                                </div>
                            ))}
