...
const GM = () => {
    ...
    useEffect(() => {
        const fetchGmData = async () => {
            try {
                const response = await axios.get('/api/auth/gm'); // Adjust the API endpoint as per your backend route
                const gmData = response.data;

                const gmDataWithImages = gmData.map(data => ({
                    ...data,
                    image1: `data:image/png;base64,${data.image1}`,
                    image2: `data:image/png;base64,${data.image2}`
                }));

                setGmData(gmDataWithImages);

                if (gmDataWithImages.length > 0) {
                    setPageTitle(gmDataWithImages[0].title);
                }
            } catch (error) {
                console.error('Error fetching General Medicine data:', error);
                setError('Failed to fetch General Medicine data');
            }
        };
